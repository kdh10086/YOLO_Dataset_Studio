...
def check_for_quit_key():
    ...
    if platform.system() == "Windows":
        while not stop_training_flag:
            if msvcrt.kbhit() and msvcrt.getch().decode(errors='ignore').lower() == 'q': stop_training_flag = True; break
    else: # Linux/macOS
...
def display_training_results(results_dir):
    ...
    try:
        for f in ['results.png', 'confusion_matrix.png']:
            ...
            if os.path.exists(img_path): Image.open(img_path).show()
    except Exception as e: print(f"Error: An error occurred while displaying graphs: {e}")
...
def train_model(config, args):
    ...
    student_cfg = config['model_configurations']['student_model_config']
    # Hyperparameters default to the teacher's if not specified for the student
    h_params = student_cfg.get('hyperparameters', config['model_configurations']['teacher_model_config']['hyperparameters'])
    
    dataset_dir_rel = args.dataset or INIT_DATASET_DIR or config['dataset_paths']['final_training_dataset']
    ...
    print("\n" + "="*50); print("Starting YOLO Model Training. (Phase 3: Final Student)"); print("="*50)
    ...
    model = YOLO(f"{model_name}.pt") # Student model starts training from scratch
    ...
    try:
        ...
        if not stop_training_flag: print("\nFinal Student model training completed successfully!")
        else: print("\nTraining was stopped by the user.")
        
        if results and hasattr(results, 'save_dir') and os.path.exists(os.path.join(results.save_dir, 'weights', 'best.pt')):
            final_model_path = os.path.join(results.save_dir, 'weights', 'best.pt')
            print(f"Final model saved at:\n   {final_model_path}")
            if not stop_training_flag: display_training_results(results.save_dir)

    except Exception as e: print(f"An error occurred during training: {e}")
    finally: stop_training_flag = True # Ensure thread exits
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Trains the final YOLO Student model.")
